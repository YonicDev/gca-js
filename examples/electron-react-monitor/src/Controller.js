...
export function Controller({data, currentController}) {
    const mainStick = React.useRef(null);
    const baseStick = React.useRef(null);
    const [stickDimensions, setStickDimensions] = React.useState(248*0.5);
    const [baseDimensions, setBaseDimensions] = React.useState(248*0.333);

    React.useEffect(() => {
        function onWindowResize() {
            if(mainStick.current && baseStick.current) {
                setBaseDimensions(baseStick.current.getClientRects()[0].width);
                setStickDimensions(mainStick.current.getClientRects()[0].width);
            }
        }
    ...
    function getDualAxisTransform(horizontal, vertical) {
        return `translate(${0.666 * horizontal * (stickDimensions - baseDimensions)}px, ${-0.666 * vertical * (stickDimensions - baseDimensions)}px)`;
    }

    function getSingleAxisBar(axis) {
        return `${(axis+1)*50}%`
    }
    ...
    return (
        <div className="controllerDisplay">
            ...
            <svg style={{opacity: isConnected? 1 : 0.5}} id="controller" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 686 496">
                ...
                <polyline id="CStickBase" ref={mainStick}  points="432.45 243.752 401.524 256.657 388.279 287.528 401.185 318.454 432.629 331.2 463.556 318.294 475.734 287.351 462.828 256.424" style={{fill: "#ffc82c"}}/>
...
export function ControllerChooser({onIncrement, onDecrement, currentController}) {
    return (
